...
import type { GuildSettings, ManagedSetupStep, PrismaClient } from '../generated/prisma/client.js';
...
export class GuildResourceService {
  ...
  public async disable(
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      const settings = await transaction.guildSettings.findUnique({ where: { guildId } });
      if (settings === null)
        throw new PublicError('GUILD_NOT_CONFIGURED', 'This server is not configured.');
      if (!settings.enabled) return false;
      await transaction.guildSettings.update({
        where: { guildId },
  ...
  public async enable(
    ...
    const settings = await this.prisma.guildSettings.findUnique({ where: { guildId } });
    if (settings === null)
      throw new PublicError('GUILD_NOT_CONFIGURED', 'This server is not configured.');
    if (
    ...
    return this.prisma.$transaction(async (transaction) => {
      await transaction.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${guildId}, 0))`;
      const updated = await transaction.guildSettings.updateMany({
  ...
  private assertSetupAvailable(settings: GuildSettings | null): void {
  ...
  private async reserveSetup(
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      const current = await transaction.guildSettings.findUnique({
        where: { guildId: command.guildId },
      });
      ...
      const saved = await transaction.guildSettings.upsert({
        where: { guildId: command.guildId },
        create: {
  ...
  private async deleteTracked(
    ...
    await this.assertManageChannels(guild);
    let settings = await this.prisma.guildSettings.findUnique({ where: { guildId } });
    if (settings === null) return;
    ...
    settings = await this.prisma.guildSettings.findUnique({ where: { guildId } });
    ...
    await this.prisma.$transaction(async (transaction) => {
      await transaction.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${guildId}, 0))`;
      const current = await transaction.guildSettings.findUnique({ where: { guildId } });
      if (
        current === null ||
  ...
  private async requireManagedSettings(guildId: string): Promise<GuildSettings> {
    const settings = await this.prisma.guildSettings.findUnique({ where: { guildId } });
    if (
      settings === null ||
      settings.managedResourceState === 'NONE' ||
  ...
  private preview(settings: GuildSettings): ManagedPreview {
    return {
  ...
  private async validateExisting(settings: GuildSettings): Promise<void> {
